...
def getItalian():
    try:
        ...
        g.conn.execute(
            "INSERT INTO Dining_Hall_Orders(item_name, bill_amount, category, order_on, residentid) VALUES ( %s, %s, "
            "%s, %s, %s)",
            args)
...
def getSnacks():
    try:
        ...
        args = (item_name, bill_amount, category, order_on, residentid)
        g.conn.execute(
            "INSERT INTO Dining_Hall_Orders(item_name, bill_amount , category, order_on, residentid ) VALUES ( %s, "
            "%s, %s, %s, %s)",
            args)
        dining_credit = dining_credit - bill_amount
        g.conn.execute(
...
def admissions_employee():
    try:
        ...
        cursor_vacant_rooms = g.conn.execute(
            "SELECT r.room_number, COALESCE(r1.to_date, CURRENT_DATE), r.room_type "
...
def admission_approved():
    try:
        ...
        new_status = 'Approved'
        old_status = 'Pending'
        all_application_id = g.conn.execute(
            "SELECT applicationid FROM Applicants_ApprovedBy WHERE approval_status=%s",
            old_status)
        ...
        date_vacant_of_entered_room = g.conn.execute("SELECT COALESCE(r1.to_date, CURRENT_DATE) "
                                                     "FROM Rooms r LEFT JOIN Residents r1 ON "
                                                     "r.room_number=r1.room_number WHERE r.room_number=%s",
                                                     room_number)
        ...
        g.conn.execute(
            "INSERT INTO Residents (residentid, name, citizenship, passport_number, date_of_birth, gender, "
...
def finance_approved():
    try:
        ...
        if int(request_id) not in get_all_request_ids_for_employee:
            ...
            new_status = "Approved"
            result = g.conn.execute("UPDATE Requests SET request_status=%s WHERE requestid=%s",
